...
const pwaSettings = {};
const version = 20; // version of this file

// should any assets be cached at all
pwaSettings.shouldUseCache = true;

// version of site assets, should increase every publish
pwaSettings.SITE_VERSION = toHash('1560267052000');

// version of runtime files, should increase on every deploy
pwaSettings.RUNTIME_VERSION = toHash('2019-06-06T09_19_38');

// version of this file
pwaSettings.SERVICE_WORKER_VERSION = '1' + '_' + version;
 ...
 */
function cacheFirst(cacheName, { postFetch, shouldForceNetwork } = {}) {
  return async function({ event, params = {} }) {
    const request = event.request;
    
    // skip the cache entirely if needed
    ...
    const forceNetwork = shouldForceNetwork ? await shouldForceNetwork(request, params) : false;
...
function addParamsNative(url = '', params = {}) {
  ...
  const searchStr = search.concat(Object.keys(params).map(k => {
    const v = params[k];
...
self.addEventListener("activate", event => {
  ...
  if(pwaSettings.shouldUseCache) {
    logGroup('using caches:', [SITE_ASSETS_CACHE_NAME, RUNTIME_CACHE_NAME, SITE_PAGES_CACHE_NAME, FONTS_CACHE_NAME])
  } else {
...
if(pwaSettings.shouldUseCache) {
  ...
  workbox.routing.registerRoute(
    ...
    cacheFirst(SITE_PAGES_CACHE_NAME, { 
      ...
      shouldForceNetwork: shouldForceNetwork
    })
  )
  ...
  // fonts
  workbox.routing.registerRoute(
    new RegExp('.*?fonts.(?:googleapis|gstatic).com/(.*)'),
...
function matchSitePages({url, event}) {
  const request = event.request;
  if(request) {
    if(request.mode === 'navigate') { // direct navigation
      return { ajaxNav: false, sitePage: true };
    } else if(isAjaxNav(request.url)) { // ajax navigation
...
async function shouldForceNetwork(request, params = {}) {
  try {
    if(!params.sitePage) {
      return false;
    }
    const pageUrl = request.url;
    const keyResponse = await fetchPageKey(pageUrl);
    if(keyResponse) {
      const strippedUrl = removeAjax(pageUrl);
      const newCacheKey = (await keyResponse.json()).value;
      const cache = await caches.open('PagesCacheKeys');
      const currentCacheKeyResp = await cache.match(strippedUrl);
      const currentCacheKey = currentCacheKeyResp ? (await currentCacheKeyResp.json()).cacheKey : '';
      if(currentCacheKey === newCacheKey) {
        log('found equal keys');
        return false;
      } else {
        log('different cache keys');
        await cache.put(strippedUrl, createResponseFromJson({ cacheKey: newCacheKey }));
        return true;
      }
    }
  } catch (err) {
